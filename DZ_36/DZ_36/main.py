...
{"name": "Партнёры", 'url': 'infopartners'},
...
def renault():
    db = get_db()
    dbase = FDataBase(db)
    print(url_for('renault'))
    return render_template('renault.html', title="Renault", dbs=dbase.get_menu(), menu=menu)

@app.route("/infopartners")
def infopartners():
    print(url_for('infopartners'))
...
def bmw():
    ...
    dbase = FDataBase(db)
    print(url_for('bmw'))
    return render_template('bmw.html', title="Bmw", dbs=dbase.get_menu(), menu=menu)

@app.route("/mercedes")
def mercedes():
    db = get_db()
    dbase = FDataBase(db)
    return render_template('mercedes.html', title="Mercedes", dbs=dbase.get_menu(), menu=menu)
